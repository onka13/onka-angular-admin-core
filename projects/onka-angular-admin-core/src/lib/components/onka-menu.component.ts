...
export type OnkaMenuItem = { name: string; icon: string; label?: string; subMenus?: OnkaPageConfig[] };
...
@Component({
  ...
  template: `
    <mat-accordion [togglePosition]="'before'">
      <mat-list>
        <mat-nav-list>
          <a mat-list-item [routerLink]="['/']" routerLinkActive="list-item-active" [routerLinkActiveOptions]="{ exact: true }">
          ...
          <mat-expansion-panel *ngFor="let menu of menuList; index as i">
            ...
            <ng-container *ngFor="let pageConfig of menu.subMenus; index as j">
              <a mat-list-item [routerLink]="[pageConfig.route]" routerLinkActive="list-item-active">
...
export class OnkaMenuComponent implements OnInit, AfterViewInit {
  ...
  initMenu(): void {
    console.log('allModuleConfigs', this.allModuleConfigs);
    console.log('menuList', this.menuList);

    for (let i = 0; i < this.menuList.length; i++) {
      const menu = this.menuList[i];
      this.menuList[i].subMenus = this.getSubRoutes(this.menuList[i].name);
      this.menuList[i].label = this.onkaService.translate('menu.' + this.menuList[i].name);
    }
  }
