      ...
      <div class="onka-dialog-content">
        ...
        <div mat-dialog-actions>
          <ng-container *ngFor="let action of data.actions; index as i">
            <button mat-button (click)="onActionClick(action)" [color]="action.color">{{ action.label }}</button>
          </ng-container>
        </div>
      </div>
    </ng-container>
    <iframe *ngIf="url" style="overflow:hidden;height:100%;width:100%" height="100%" width="100%" frameborder="0" [src]="url"></iframe> `,
})
